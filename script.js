...
async function getWeather(inputValue) {
    try {
       ...
       const D_feels = Math.round(data.main.feels_like)
       const humidity = data.main.humidity
       const wind_speed = data.wind.speed;

       weather_data.querySelector(".ico").innerHTML = `<img src="http://openweathermap.org/img/wn/${icon}.png" alt="">`

       weather_data.querySelector(".temp").textContent = `${temp}°c`
       weather_data.querySelector(".clouds").textContent = `${description}`
       weather_data.querySelector(".details .Feels").textContent = `Feels like ${D_feels}`
       weather_data.querySelector(".details .Humidity").textContent = `Humidity ${humidity}`
       weather_data.querySelector(".details .w-speed").textContent = `Wind speed ${wind_speed}`
